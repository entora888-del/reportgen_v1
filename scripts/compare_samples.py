...
def find_liq_pdf(sample_dir: Path) -> Path | None:
    for name in ("液状化.pdf", "*液状化*.pdf"):
        hits = sorted(sample_dir.glob(name))
        if hits:
            return hits[0]
    return None
...
def main() -> int:
    ...
    parser.add_argument("--exclude", default="", help="Comma-separated sample folder names to skip (e.g., 10)")
    parser.add_argument("--template", default=None, help="Template path to force (otherwise resolve_template_path(None))")
